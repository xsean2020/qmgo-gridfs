...
type GridFS struct {
	Files  *qmgo.Collection
	Chunks *qmgo.Collection
}
...
func (gfs *GridFS) Create(name string) (file *GridFile, err error) {
	...
	file.wsum = md5.New()
	file.doc = gfsFile{Id: primitive.NewObjectID(), ChunkSize: 255 * 1024, Filename: name}
	return
}
...
//     check(err)
//
func (gfs *GridFS) OpenId(ctx context.Context, id interface{}) (file *GridFile, err error) {
	var doc gfsFile
	err = gfs.Files.Find(ctx, bson.M{"_id": id}).One(&doc)
	if err != nil {
		return
	}
	file = gfs.newFile()
...
//     check(err)
//
func (gfs *GridFS) Open(ctx context.Context, name string) (file *GridFile, err error) {
	var doc gfsFile
	err = gfs.Files.Find(ctx, bson.M{"filename": name}).Sort("-uploadDate").One(&doc)
	if err != nil {
		return
	}
	file = gfs.newFile()
...
//     }
//
func (gfs *GridFS) OpenNext(iter qmgo.CursorI, file **GridFile) bool {
	if *file != nil {
...
//
func (gfs *GridFS) Find(ctx context.Context, query interface{}) qmgo.QueryI {
	return gfs.Files.Find(ctx, query)
}

// RemoveId deletes the file with the provided id from the GridFS.
func (gfs *GridFS) RemoveId(ctx context.Context, id interface{}) error {
	err := gfs.Files.Remove(ctx, bson.M{"_id": id})
	if err != nil {
		return err
	}
	_, err = gfs.Chunks.RemoveAll(ctx, bson.D{{Key: "files_id", Value: id}})
	return err
}
...
func (gfs *GridFS) Remove(ctx context.Context, name string) (err error) {
	iter := gfs.Files.Find(ctx, bson.M{"filename": name}).Select(bson.M{"_id": 1}).Cursor()
	var doc gfsDocId
	for iter.Next(&doc) {
		if e := gfs.RemoveId(ctx, doc.Id); e != nil {
			err = e
		}
	}
	if err == nil {
...
func (file *GridFile) GetMeta(result interface{}) (err error) {
	...
	if file.doc.Metadata != nil {
		err = bson.Unmarshal(*file.doc.Metadata, result)
	}
	file.m.Unlock()
	return
}
...
func (file *GridFile) completeWrite() {
	...
	if file.err == nil {
		...
		if file.doc.UploadDate.IsZero() {
			file.doc.UploadDate = time.Now()
		}
		file.doc.MD5 = hexsum
		_, file.err = file.gfs.Files.InsertOne(context.Background(), file.doc)
	}
	if file.err != nil {
		file.gfs.Chunks.RemoveAll(ctx, bson.D{{Key: "files_id", Value: file.doc.Id}})
	}
	if file.err == nil {
		...
		file.err = file.gfs.Chunks.CreateOneIndex(ctx, index)
	}
}
...
func (file *GridFile) insertChunk(data []byte) {
	...
	data, err := bson.Marshal(gfsChunk{primitive.NewObjectID(), file.doc.Id, n, data})
	if err != nil {
		file.err = err
		return
	}
	...
	go func() {
		_, err := file.gfs.Chunks.InsertOne(context.Background(), bson.Raw(data))
		file.m.Lock()
		file.wpending--
		if err != nil && file.err == nil {
			file.err = err
...
func (file *GridFile) getChunk() (data []byte, err error) {
	...
	if cache != nil && cache.n == file.chunk {
		...
		var doc gfsChunk
		err = file.gfs.Chunks.Find(context.Background(), bson.D{{Key: "files_id", Value: file.doc.Id}, {Key: "n", Value: file.chunk}}).One(&doc)
		data = doc.Data
	}
	file.chunk++
	if int64(file.chunk)*int64(file.doc.ChunkSize) < file.doc.Length {
		...
		go func(id interface{}, n int) {
			var doc gfsChunk
			cache.err = chunks.Find(context.Background(), bson.D{{Key: "files_id", Value: id}, {Key: "n", Value: n}}).One(&doc)
			cache.data = doc.Data
			cache.wait.Unlock()
